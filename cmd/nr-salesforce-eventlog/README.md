...
All events and logs generated by this integration contain an attribute named
`sf.instance.name`, which is the value of [`instanceName`](#--instancename).
This attribute can be used to filter data from different instances, either using
the filter bar in a dashboard, or the `WHERE` clause in a NRQL. Filtering by
`sf.instance.name` is very important when multiple instances send data to the
same New Relic account, to avoid visualizing scrambled non-sense data.
